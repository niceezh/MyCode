...
def is_player_winner(board, player):
    ...
    if board[0][2] == board[1][1] == board[2][0] == player:
        return True
    return any(board[0][col] == board[1][col] == board[2][col] == player for col in range(3))
...
def gamestate(board):
    ...
    """
    count_moves_x = count_player_moves(board, 'X')
    count_moves_o = count_player_moves(board, 'O')
    is_winner_x = is_player_winner(board, 'X')
    is_winner_o = is_player_winner(board, 'O')
    if count_moves_o > count_moves_x:
        raise ValueError('Wrong turn order: O started')
    if count_moves_x - count_moves_o > 1:
        raise ValueError('Wrong turn order: X went twice')
    if is_winner_x and is_winner_o:
        raise ValueError('Impossible board: game should have ended after the game was won')
    if is_winner_x or is_winner_o:
        return 'win'
    if count_moves_x + count_moves_o == 9:
        return 'draw'
    return 'ongoing'
