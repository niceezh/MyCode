...
def tick(matrix):
    ...
    for i in range(1, row_size+1):
        for j in range(1, col_size+1):
            live_neighbors = padded_matrix[i-1][j-1] + padded_matrix[i-1][j] + padded_matrix[i-1][j+1] + padded_matrix[i][j-1] + padded_matrix[i][j+1] + padded_matrix[i+1][j-1] + padded_matrix[i+1][j] + padded_matrix[i+1][j+1]
            if live_neighbors == 2:
                continue
            if live_neighbors == 3:
                matrix[i-1][j-1] = 1
